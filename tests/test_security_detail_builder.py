...
class FakeProvider:
    ...
    def get_financial_summary(self, code: str) -> pd.DataFrame:
        return pd.DataFrame(
            [
                {"disclosed_date": "2025-08-01", "net_sales": 100, "operating_profit": 10},
                {"disclosed_date": "2026-08-01", "net_sales": 120, "operating_profit": 15},
            ]
        )
...
def test_recommendation_separates_fundamental_and_technical() -> None:
    block = recommendation_block(
        ...
        {
            "fundamental": {"positive_reasons": ["品質"], "risk_reasons": ["欠損"]},
            "technical": {
...
def test_builder_writes_sanitized_detail_files(tmp_path: Path, monkeypatch: object) -> None:
    web = tmp_path / "web"
    (web / "data" / "paper-trading").mkdir(parents=True)
    (web / "jquants-ranking.json").write_text(
        '{"metadata":{"effective_data_cutoff":"2026-05-11"},'
        '"rows":[{"rank":1,"code":"80350","company_name":"東京エレクトロン"}]}',
        encoding="utf-8",
    )
    (web / "data" / "paper-trading" / "latest-report.json").write_text(
        '{"decisions":[{"code":"8035","fundamental":{},"technical":{},'

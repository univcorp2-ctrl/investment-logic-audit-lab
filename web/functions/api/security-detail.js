...
const JQUANTS_ENDPOINTS = Object.freeze({
  summary: '/v2/fins/summary',
  details: '/v2/fins/details',
  earnings: '/v2/fins/earnings-date',
  tdnet: '/v2/td/list',
});
...
function findByCode(rows, code) { return (rows ?? []).find(row => code4(row.code ?? row.symbol) === code) ?? null; }
...
async function yahooChart(code) {
  ...
  return timestamps.map((timestamp, index) => ({ date:new Date(Number(timestamp) * 1000).toISOString().slice(0, 10), open:finite(quote.open?.[index]), high:finite(quote.high?.[index]), low:finite(quote.low?.[index]), close:finite(quote.close?.[index]), adjusted_close:finite(adjusted?.[index]), volume:finite(quote.volume?.[index]) })).filter(row => row.open !== null && row.high !== null && row.low !== null && row.close !== null);
}
function parseGeneralNewsRss(xml, limit = 8) {
  ...
  return [...String(xml).matchAll(/<item>([\s\S]*?)<\/item>/gi)].slice(0, limit).map(match => {
    ...
    return { title:tag(body, 'title'), source:decodeXml(source?.[2] ?? '') || 'Google News', published_at:tag(body, 'pubDate') || null, link:tag(body, 'link') };
...
function normalizeFinancial(row) {
  ...
  return { code:get('Code','code'), disclosure_date:get('DisclosedDate','DiscDate','disclosed_date','disclosure_date'), period_end:get('CurrentPeriodEndDate','CurPerEn','period_end'), fiscal_year_end:get('CurrentFiscalYearEndDate','FYE','fiscal_year_end'), document_type:get('TypeOfDocument','DocType','document_type'), period_type:periodType(row), sales:finite(get('NetSales','Sales','net_sales','sales')), operating_profit:finite(get('OperatingProfit','OP','operating_profit')), ordinary_profit:finite(get('OrdinaryProfit','OdP','ordinary_profit')), net_profit:finite(get('Profit','net_profit','profit')), eps:finite(get('EarningsPerShare','EPS','eps')), total_assets:finite(get('TotalAssets','TA','total_assets')), equity:finite(get('Equity','Eq','equity')), operating_cash_flow:finite(get('CashFlowsFromOperatingActivities','CFO','operating_cash_flow')), investing_cash_flow:finite(get('CashFlowsFromInvestingActivities','CFI','investing_cash_flow')), financing_cash_flow:finite(get('CashFlowsFromFinancingActivities','CFF','financing_cash_flow')) };
}
function changes(current, previous) {
  const growth = field => current.period_type === previous?.period_type && current[field] !== null && previous?.[field] !== null && previous?.[field] !== 0 ? Math.round(((current[field] / Math.abs(previous[field]) - Math.sign(previous[field])) * 100) * 1e12) / 1e12 : null;
...
function splitReasons(decision) {
  ...
  return { fundamental_reasons_positive:[...(fundamental.positive_reasons ?? [])], fundamental_risks:[...(fundamental.risk_reasons ?? []), ...(fundamental.missing ?? []).map(name => `欠損: ${name}`)], technical_reasons_positive:[...(technical.positive_reasons ?? [])], technical_risks:[...(technical.risk_reasons ?? []), ...(technical.missing ?? []).map(name => `欠損: ${name}`)] };
}
function localFinancialRows(local) { return (local?.financial_summaries ?? []).map(normalizeFinancial); }
export async function onRequestGet(context) {
  ...
  const [index, ranking, report, local] = await Promise.all([assetJson(context, '/data/stock-details/index.json', {securities:[]}), assetJson(context, '/jquants-ranking.json', {rows:[],metadata:{}}), assetJson(context, '/data/paper-trading/latest-report.json', {decisions:[]}), assetJson(context, `/data/stock-details/${code}.json`, {})]);
  ...
  const jqCode = `${code}0`;
  const [chartSettled, newsSettled, summary, details, earnings, tdnet] = await Promise.all([Promise.allSettled([yahooChart(code)]).then(([result]) => result), Promise.allSettled([generalNews(name, code)]).then(([result]) => result), jqAll(context.env, JQUANTS_ENDPOINTS.summary, { code:jqCode }), jqAll(context.env, JQUANTS_ENDPOINTS.details, { code:jqCode }), jqAll(context.env, JQUANTS_ENDPOINTS.earnings, { code:jqCode }), jqAll(context.env, JQUANTS_ENDPOINTS.tdnet, { code:jqCode })]);
  ...
  const payload = { generated_at:new Date().toISOString(), code, name, security:{ code, symbol:`${code}.T`, company_name:name, market:rankingRow.market ?? known.market ?? null, sector:rankingRow.sector ?? known.sector ?? null }, fundamental:{ score:fundamental.score ?? rankingRow.fundamental_score ?? null, value_score:fundamental.value_score ?? rankingRow.value_score ?? null, quality_score:fundamental.quality_score ?? rankingRow.quality_score ?? null, growth_stability_score:fundamental.growth_stability_score ?? rankingRow.growth_stability_score ?? null, value_trap_risk:fundamental.value_trap_risk ?? rankingRow.value_trap_risk ?? null, data_completeness:fundamental.data_completeness ?? rankingRow.data_completeness ?? null, earnings_yield:fundamental.earnings_yield ?? null, book_to_market:fundamental.book_to_market ?? null, fcf_yield:fundamental.fcf_yield ?? null, roe:fundamental.roe ?? null, operating_margin:fundamental.operating_margin ?? null }, technical:{ score:technical.score ?? rankingRow.technical_score ?? null, price:technical.price ?? rankingRow.last_price ?? null, sma20:technical.sma20 ?? null, sma60:technical.sma60 ?? null, rsi14:technical.rsi14 ?? null, momentum20_pct:technical.momentum20_pct ?? null, momentum60_pct:technical.momentum60_pct ?? null, volatility20_pct:technical.volatility20_pct ?? null, drawdown20_pct:technical.drawdown20_pct ?? null }, recommendation, holding:decision.holding ?? {quantity:0,avg_cost:null}, quote:decision.quote ?? {}, financials:{ status:remotePeriods.length ? summary.status : periods.length ? 'local_sanitized' : summary.status, periods, details_status:details.status, latest_details:details.rows[0] ?? null, full_statements_entitled:details.status !== 'not_entitled', full_statements_available:Boolean(details.rows.length) }, earnings:{ status:earnings.rows.length ? earnings.status : earningsHistory.length ? 'local_sanitized' : earnings.status, history:earningsHistory, next:earningsHistory[0] ?? null }, disclosures:{ status:disclosures.length ? (tdnet.rows.length ? tdnet.status : 'local_sanitized') : (local?.official_disclosure_status ?? tdnet.status), items:disclosures, search_url:`https://www.google.com/search?q=${encodeURIComponent(`${name} ${code} 適時開示 決算短信`)}` }, general_news:newsItems, chart:{ status:chartRows.length ? 'ok' : (chartSettled.reason?.name === 'AbortError' ? 'timeout' : 'unavailable'), rows:chartRows }, data_dates:{ effective_fundamental_cutoff:report.fundamental_source?.effective_data_cutoff ?? ranking.metadata?.effective_data_cutoff ?? null, latest_disclosure_date:fundamental.latest_disclosure_date ?? periods[0]?.disclosure_date ?? null, latest_price_date:chartRows.at(-1)?.date ?? technical.price_date ?? null }, source_status:{ chart:{ok:Boolean(chartRows.length),error:chartSettled.status === 'rejected' ? String(chartSettled.reason?.message ?? chartSettled.reason) : null}, general_news:{ok:Boolean(newsItems.length),error:newsSettled.status === 'rejected' ? String(newsSettled.reason?.message ?? newsSettled.reason) : null}, financial_summary:{status:summary.status,local_fallback:!remotePeriods.length && periods.length>0}, financial_details:{status:details.status}, earnings:{status:earnings.status,local_fallback:!earnings.rows.length && earningsHistory.length>0}, tdnet:{status:tdnet.status,local_fallback:!tdnet.rows.length && localDisclosures.length>0} }, warnings, paper_only:true };

export const PHONE_DESTINATIONS = Object.freeze({
  overview: Object.freeze([
    ...
    { key:'freshness', label:'データ鮮度', purpose:'データ日付と利用可能範囲を確認', selectors:['#dataNotice','#dataError'] },
  ]),
  decision: Object.freeze([
    ...
    { key:'ranking', label:'ランキング', purpose:'候補銘柄を順位とスコアで比較', selectors:['#adaptiveLegacyDetails','.ranking'], openDetails:'#adaptiveLegacyDetails' },
  ]),
  ...
  other: Object.freeze([
    ...
    { key:'strategy', label:'戦略ラボ・自動学習', purpose:'研究候補・学習ゲート・過学習警告を確認', selectors:['#autoLearningPanel','#investmentDecisionReport'], decisionTab:'strategy' },
...
export function normalizePhoneDestination(value) { return Object.hasOwn(PHONE_DESTINATIONS, value) ? value : 'overview'; }
export function normalizePhoneSubpage(destination, value) { const pages=PHONE_DESTINATIONS[normalizePhoneDestination(destination)]; return pages.some(page=>page.key===value)?value:pages[0].key; }
export function parsePhoneHash(hash='') { const params=new URLSearchParams(String(hash).replace(/^#/,'')); const raw=params.get('phone')??''; const [destinationRaw,subpageRaw]=raw.split(':'); const destination=normalizePhoneDestination(destinationRaw); return {destination,subpage:normalizePhoneSubpage(destination,subpageRaw),params}; }
export function serializePhoneHash(destination,subpage,currentHash='') { const parsed=parsePhoneHash(currentHash); const normalizedDestination=normalizePhoneDestination(destination); const normalizedSubpage=normalizePhoneSubpage(normalizedDestination,subpage); parsed.params.set('phone',`${normalizedDestination}:${normalizedSubpage}`); return `#${parsed.params.toString()}`; }
export function pageIndex(destination,subpage) { const pages=PHONE_DESTINATIONS[normalizePhoneDestination(destination)]; return Math.max(0,pages.findIndex(page=>page.key===normalizePhoneSubpage(destination,subpage))); }
export function adjacentPhonePage(destination,subpage,direction) { const pages=PHONE_DESTINATIONS[normalizePhoneDestination(destination)]; const index=pageIndex(destination,subpage); return pages[Math.min(pages.length-1,Math.max(0,index+(direction<0?-1:1)))]; }
export function allManagedSelectors() { return [...new Set(Object.values(PHONE_DESTINATIONS).flat().flatMap(page=>page.selectors))]; }

...
def evaluate_post_promotion_guard(
    root: Path,
    config: PostPromotionGuardConfig = PostPromotionGuardConfig(),
) -> dict[str, Any]:
    ...
    if (
        ...
        or state.get("previous_profile") != previous.get("name")
        or "promotion_observations" not in state
    ):
        state = {
            **result,
            "status": "monitoring",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Post-promotion relative performance guard for the paper strategy"
    )
